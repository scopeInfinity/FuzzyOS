...
int verify_partition(int partition_id) {
    ...
    if (partition_id < 0 || partition_id >= 4)
        return 1;
    if (verified_cache[partition_id] != -1)
        return verified_cache[partition_id];
    ...
    int err = partition_read_block(partition.lba, // FS metadata
                                   block.bytes);
    if (!err) {
        // verify signature
    ...
    if (err) {
        ...
        print_log("[ffs] partition %d verification failed, err: %d",
                  partition_id, err);
        return err;
    }
...
int fetch_file_entry(int partition_id, int entry_id,
    ...
    err = partition_read_block(partition.lba + 1 + // FS metadata
...
int fetch_file_content(const int partition_id, const union FFSFileEntry *entry,
    ...
    int err = partition_read_block(
        partition.lba + entry->content.start_block_id + file_block_id, buffer);
    if (err)
        return -2;
    // returns content length in buffer.
    if (file_block_id + 1 == block_count)
        return file_size % FS_BLOCK_SIZE;
