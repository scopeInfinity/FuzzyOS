...
void load_kernel() {
    ...
    if(err) {
        ...
        print_log("Kernel loaded at 0x%x: %s...", MEMORY_LOCATION_KERNEL, get_memdump_8byte((void*)MEMORY_LOCATION_KERNEL));
    }
}
...
void load_static_library() {
    ...
    if(err) {
        ...
        print_log("Static library loaded at 0x%x: %s...", MEMORY_STATIC_LIBRARY, get_memdump_8byte((void*)MEMORY_STATIC_LIBRARY));
    }
}
